...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_padded = torch.tensor(pad_sents_char(sents_charindices, self.char_pad), device=device) # (batch_size, max_sentence_length, max_word_length)
        sents_var = sents_padded.transpose(0, 1)
